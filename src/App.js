...
// Log missing env vars for debugging (prevents silent failures on Vercel)
if (!process.env.REACT_APP_FIREBASE_API_KEY) console.warn('Missing Firebase API Key');
if (!process.env.REACT_APP_CLOUDINARY_CLOUD_NAME) console.warn('Missing Cloudinary Cloud Name');
        ...
        <div className="overflow-y-auto max-h-96 pr-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select
                id="paymentType"
                value={newCylinder.paymentPolicy.type}
                onChange={(e) => setNewCylinder({ ...newCylinder, paymentPolicy: { ...newCylinder.paymentPolicy, type: e.target.value } })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                <option value="percentage">Percentage</option>
                <option value="fixed_amount_per_order">Fixed Amount</option>
              </select>
            </div>
            <div>
              <label htmlFor="paymentValue" className="block text-sm font-medium text-gray-700">Payment Value</label>
              <input
                id="paymentValue"
                type="number"
                placeholder="Payment Value"
                value={newCylinder.paymentPolicy.value}
                onChange={(e) => setNewCylinder({ ...newCylinder, paymentPolicy: { ...newCylinder.paymentPolicy, value: e.target.value } })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                required
              />
              {validationErrors.paymentPolicyValue && <p className="text-red-500 text-xs mt-1">{validationErrors.paymentPolicyValue}</p>}
            </div>
          </div>
          <div>
        ...
        {selectedCylinderForPayment && (
          <div className="space-y-2 p-4 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-bold text-red-500">{currencySymbol}{calculateBalance(selectedCylinderForPayment)}</span>
            </div>
          </div>
        )}
        <div>
          <label htmlFor="quickPaymentAmount" className="block text-sm font-medium text-gray-700">Payment Amount</label>
          <input
...
const SupplierCylinderListModal = ({ selectedSupplier, cylinders, onClose, currencySymbol, exportToPdf }) => {
  ...
  const filteredCylinders = supplierCylinders.filter(c => 
    ...
    (c.cylinderDate ? new Date(cylinder.cylinderDate.seconds * 1000).toLocaleDateString('en-GB').includes(searchQuery) : false)
  );
...
const App = () => {
  ...
  const handleGenerateReminder = (customer, cylinders) => {
    ...
    const message = `Dear ${customer.name},\n\nPlease be reminded of your outstanding balance of ${currencySymbol}${totalOutstanding} for the following cylinders:\n\n${customerCylinders.map(c => `- ${c.name}: ${currencySymbol}${c.balance}`).join('\n')}\n\nKindly settle the amount at your earliest convenience.\n\nThank you,\nCylinder Tracker Team`;
                ...
                {cylinders.map(cylinder => (
                  <div key={cylinder.id} className="bg-gray-50 p-4 rounded-lg flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">Balance: {currencySymbol}{cylinder.balance}</p>
                    </div>
                ...
                {customers.map(customer => (
                  <div key={customer.id} className="bg-gray-50 p-4 rounded-lg flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">Contact: {customer.contactInfo}</p>
                    </div>
                    <div className="flex space-x-2">
                      <button
                        onClick={() => {
                ...
                {suppliers.map(supplier => (
                  <div key={supplier.id} className="bg-gray-50 p-4 rounded-lg flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">Contact: {supplier.contactInfo}</p>
                    </div>
                    <div className="flex space-x-2">
                      <button
                        onClick={() => {
